...
public class Loops {
	...
	//part7 Boolean Checker true/false
	//Integer.reverse(number)
	private static boolean palindromeCheckerMath (int num) {
		int i = num;
		int reverse = 0;
		while (i != 0) {
			int tempValue = i%10;
			reverse = reverse * 10 + tempValue;
			i = i / 10;
		}
		if (num == reverse) {
			return true;
	...
	private static void part7() {
		for (int i = 10; i <= 200; i++) {
			int numberChecker = Integer.valueOf(i);
			if(palindromeCheckerMath(numberChecker)) {
